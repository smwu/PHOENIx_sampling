#================================================
# Simulation code for TB Control Sampling Project
# Updated 12/20/2021
# ===============================================
...
case_distr <- data %>% group_by(cluster) %>% summarise(clust_sum = sum(y))
K_case_table <- table(case_distr$clust_sum)

# P(X=1) = prop_table[2,3]
sim_chars <- c(OR, p_case, prop_table[2,3], p_case_cond_exp, p_case_cond_unexp, 
               K_case_table[1], K_case_table[2], sum(K_case_table[3:length(K_case_table)]))

fit_full <- geeglm(y ~ x, id=cluster, data=data, family=binomial, scale.fix=TRUE, corstr="independence")
icc_full <- fit_full$corr
